...
@torch.library.custom_op("spyre::fp8todl16", mutates_args=(), device_types="spyre")
def fp8todl16(input: torch.Tensor) -> torch.Tensor:
    """
    FP8 to FP16 format conversion (dequantization without scaling).

    Converts FP8 E4M3 tensor to FP16 format.
    This operation ONLY performs format conversion - scaling must be done separately.

    Args:
        input: Input tensor (FP8) to convert to FP16

    Returns:
        FP16 tensor (same shape as input)

    Maps to: deeptools fp8todl16 operation
    """
    pass


@fp8todl16.register_fake
def _(input: torch.Tensor) -> torch.Tensor:
