...
# ============================================================================
# FP8 Quantization Lowering
# ============================================================================
# These lowering functions implement the 5-step FP8 quantization flow:
#
...
#
# All operations map to hardware primitives for maximum efficiency.
#
# User API: torch.ops.spyre.quantize_fp8(tensor) -> (fp8_tensor, scale)


@register_spyre_lowering(torch.ops.spyre.quantize_fp8)
def lower_quantize_fp8(x):
    """
    Lower quantize_fp8 operation.
    
    Implements the 5-step quantization flow:
    1. quantscalepertokenfp8 - Compute scale (REDUCTION)
    2. reciprocal - Compute 1/scale (POINTWISE, sfp)
    3. mul - Scale input (POINTWISE)
    4. clamp - Clamp to [-448, 448] (POINTWISE)
    5. qfp8 - Format conversion (POINTWISE)
    
    Returns:
        tuple: (fp8_tensor, scale)
    """
    x.realize()
    
    # Step 1: Compute scale using hardware REDUCTION operation
    scale = lower_quantscalepertokenfp8(x)
    
    # Step 2: Compute inverse scale using hardware reciprocal (sfp unit)
    inv_scale = lower_reciprocal(scale)
    
    # Step 3: Multiply by inverse scale (POINTWISE)
    x_scaled = lowering.mul(x, inv_scale)
    
    # Step 4: Clamp to FP8 range (POINTWISE)
    x_clamped = lowering.clamp(
        x_scaled,
        lowering.constant(-448.0, x.get_device()),
        lowering.constant(448.0, x.get_device())
    )
    
    # Step 5: Format conversion (POINTWISE)
    x_fp8 = lower_qfp8(x_clamped)
    
    return (x_fp8, scale)


@register_spyre_lowering(torch.ops.spyre.quantscalepertokenfp8)
def lower_quantscalepertokenfp8(x, dim=-1):
    """
    Lower quantscalepertokenfp8 as a REDUCTION operation.
    
    Computes: scale = max(abs(x)) / 448
    
    This is a REDUCTION operation that reduces along the specified dimension.
    Similar to exx2 - uses SpyreReduction, not Pointwise.
    
    Args:
        x: Input tensor
        dim: Dimension to reduce along (default: -1, last dimension)
    
    Output: Scale tensor (FP32) with specified dimension reduced to 1
    """
    ...
    kwargs = lowering._make_reduction_inner(
        x,
        axis=[dim],  # Reduce along specified dimension
        keepdims=True,
        dtype=x.get_dtype(),
        override_return_dtype=torch.float32  # Scales are always FP32
    )
    
    # Calculate output shape with specified dimension reduced to 1
    ...
    result = SpyreReduction.create(
        ...
        input_node=x,
        device=x.get_device(),
        dst_dtype=torch.float32,  # Scales are FP32
        src_dtype=x.get_dtype(),
        inner_fn=kwargs["inner_fn"],
