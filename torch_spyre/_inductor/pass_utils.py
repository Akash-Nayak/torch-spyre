...
def compute_restickify_needed(
    ...
    # ReStickifyOpHBM supports the native FP16 device format (SEN169_FP16) and
    # FP8 (SEN143_FP8). IEEE_FP32 and other non-DL16 formats are not supported.
    ...
    _RESTICKIFY_SUPPORTED_DTYPES = (DataFormats.SEN169_FP16, DataFormats.SEN143_FP8)
    ...
    if idc[-1] == sympy.S.Zero and target_stick != sympy.S.Zero:
        # The input has a sparse stick (idc[-1]=0) but the output requests a
        # real stick expression. This arises for QFP8CH activations produced
        # by a preceding matmul in the unexpanded layout (e.g. chained MLP
        # fp8_linear). compute_restickify_target_layout bails because
        # matching_dim(ic, 0) fails when idc[-1] has no free symbols.
        # out_stl is already the correct canonical target.
        return True, out_stl
